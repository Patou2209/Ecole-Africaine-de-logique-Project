...
const adsRef = ref(database, "rech");
...
const adForm = document.getElementById("article_form");
  ...
  adForm.addEventListener("submit", async (event) => {
    ...
    const tipe = document.getElementById("art-type").value;
    ...
    const saveAd = async (pdfData) => {
      ...
      if (!date || !volume || !title || !pdfData || !containt || !tipe) {
        alert("Remplissez toutes les cases Svp!.");
        return;
      } 

      if (editingAdId) {
        await update(ref(database, `rech/${editingAdId}`), ad);
        editingAdId = null;
      } else {
  ...
  window.editAd = async function(adId) {
    ...
    if (adSnap.exists()) {
      ...
      document.getElementById("art-type").value = ad.tipe;
